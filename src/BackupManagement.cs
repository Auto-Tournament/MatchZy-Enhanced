...
namespace AutoTournamentCS2
{
    public partial class AutoTournamentCS2
    {
        ...
        public string lastAutoTournamentCS2BackupFileName = "";
        ...
        {
            string backupFilePrefix = $"at_{liveMatchId}_{matchConfig.CurrentMapNumber}";
        ...
        public void OnStopCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            if (isStopCommandAvailable && isMatchLive)
            {
                if (IsHalfTimePhase())
                {
                    ...
                    ReplyToUserCommand(player, Localizer["at.backup.stopduringhalftime"]);
                    return;
                }
                if (IsPostGamePhase())
                {
                    // ReplyToUserCommand(player, "You cannot use this command after the game has ended.");
                    ReplyToUserCommand(player, Localizer["at.backup.stopmatchended"]);
                    return;
                }
                if (IsTacticalTimeoutActive())
                {
                    // ReplyToUserCommand(player, "You cannot use this command when tactical timeout is active.");
                    ReplyToUserCommand(player, Localizer["at.backup.stoptacticaltimeout"]);
                    return;
                }
                if (playerHasTakenDamage && stopCommandNoDamage.Value)
                {
                    ReplyToUserCommand(player, Localizer["at.restore.stopcommandrequiresnodamage"]);
                    return;
                }
                string stopTeamName = "";
                ...
                {
                    if (lastAutoTournamentCS2BackupFileName != "")
                    {
                        RestoreRoundBackup(player, lastAutoTournamentCS2BackupFileName);
                    }
                    else
                    {
                        // This should not happen, lastAutoTournamentCS2BackupFileName should not be empty in a live game!
                        Log($"[OnStopCommand] lastAutoTournamentCS2BackupFileName not found, unable to restore round!");
                    }

                }
                else
                {
                    PrintToAllChat(Localizer["at.restore.teamwantstorestore", stopTeamName, remainingStopTeam]);
        ...
        public void OnRestoreCommand(CCSPlayerController? player, CommandInfo command)
        {
            ...
            {
                ReplyToUserCommand(player, Localizer["at.cc.usage", "!restore <round>"]);
            }
        }
        ...
        private void HandleRestoreCommand(CCSPlayerController? player, string commandArg)
        {
            ...
            if (!string.IsNullOrWhiteSpace(commandArg))
            {
                if (int.TryParse(commandArg, out int roundNumber) && roundNumber >= 0)
                {
                    ...
                    string requiredBackupFileName = $"at_{liveMatchId}_{matchConfig.CurrentMapNumber}_round{round}.json";
                ...
                else
                {
                    ...
                    ReplyToUserCommand(player, Localizer["at.backup.restoreinvalidvalue"]);
                }
            }
            else
            {
        ...
        private void RestoreRoundBackup(CCSPlayerController? player, string fileName)
        {
            ...
            if (IsHalfTimePhase())
            {
                ReplyToUserCommand(player, Localizer["at.backup.restoreduringhalftime"]);
                return;
            }
            if (IsPostGamePhase())
            {
                ReplyToUserCommand(player, Localizer["at.backup.restorematchended"]);
                return;
            }
            if (IsTacticalTimeoutActive())
            {
                ReplyToUserCommand(player, Localizer["at.backup.restoretacticaltimeout"]);
                return;
            }
            string backupFolder = Path.Combine(Server.GameDirectory, "csgo", "AutoTournamentCS2DataBackup");
     
            string filePath = Path.Combine(backupFolder, fileName);
 
            if (!File.Exists(filePath))
            {
                ReplyToUserCommand(player, Localizer["at.backup.restoredoesntexist", fileName]);
            ...
            try
            {
                ...
                {
                    atTeam1 = Newtonsoft.Json.JsonConvert.DeserializeObject<Team>(team1config)!;
                }
                if (backupData.TryGetValue("team2", out var team2config))
                {
                    atTeam2 = Newtonsoft.Json.JsonConvert.DeserializeObject<Team>(team2config)!;
                }
                ...
                if (gameRules.WarmupPeriod)
                {
                    if (!isRoundRestorePending)
                    {
                        isRoundRestorePending = true;
                        pendingRestoreFileName = fileName;
                        PrintToAllChat(Localizer["at.restore.loadedsuccessfully", fileName]);
                        return;
                    }
                    else
                    {
                ...
                if (backupData.TryGetValue("valve_backup", out var valveBackup))
                {
                    ...
                    {
                        tempFileName = $"at_{liveMatchId}_{matchConfig.CurrentMapNumber}_round{roundNumber}.txt";
                    }
            ...
            PrintToAllChat(Localizer["at.restore.restoredsuccessfully", fileName]);
            ...
            if (backupData.TryGetValue("round", out var roundNumberStr) && int.TryParse(roundNumberStr, out int roundNum))
            {
                ...
                var backupLoadedEvent = new AutoTournamentCS2BackupLoadedEvent
                {
                    MatchId = liveMatchId,
        ...
        public void CreateAutoTournamentCS2RoundDataBackup()
        {
            Log($"[CreateAutoTournamentCS2RoundDataBackup] isRoundRestoring: {isRoundRestoring} isMatchLive: {isMatchLive}");
            ...
            try
            {
                ...
                string round = roundNumber.ToString("D2");
                string atBackupFileName = $"at_{liveMatchId}_{matchConfig.CurrentMapNumber}_round{round}.json";
                string filePath = Path.Combine(Server.GameDirectory, "csgo", "AutoTournamentCS2DataBackup", atBackupFileName);
                ...
                string lastBackupFilePath = $"at_{liveMatchId}_{matchConfig.CurrentMapNumber}_round{round}.txt"; ;
                ...
                Dictionary<string, string> roundData = new()
                    {
                        ...
                        { "team1_name", atTeam1.teamName },
                        { "team1_flag", atTeam1.teamFlag },
                        { "team1_tag", atTeam1.teamTag },
                        { "team1_side", teamSides[atTeam1] },
                        { "team2_name", atTeam2.teamName },
                        { "team2_flag", atTeam2.teamFlag },
                        { "team2_tag", atTeam2.teamTag },
                        { "team2_side", teamSides[atTeam2] },
                        { "team1_score", t1score.ToString() },
                        { "team2_score", t2score.ToString() },
                        { "team1_series_score", atTeam1.seriesScore.ToString() },
                        { "team2_series_score", atTeam2.seriesScore.ToString() },
            ...
            {
                Log($"[CreateAutoTournamentCS2RoundDataBackup FATAL] Error creating the JSON file: {e.Message}");
            }
        }

        public List<string> GetBackups(string matchID)
        {
            string backupDir = Path.Combine(Server.GameDirectory, "csgo", "AutoTournamentCS2DataBackup");
        ...
        {
            Team teamConfig = team == "team1" ? atTeam1 : atTeam2;
        ...
        [ConsoleCommand("at_loadbackup", "Restore the backup from the provided file")]
        ...
        [ConsoleCommand("at_loadbackup_url", "Loads a backup from the given URL")]
        public void LoadBackupFromURL(CCSPlayerController? player, CommandInfo command)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    string filePath = Path.Combine(Server.GameDirectory, "csgo", "AutoTournamentCS2DataBackup", fileName);
        ...
        [ConsoleCommand("at_listbackups", "List all the backups for the provided matchid")]
